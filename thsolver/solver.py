  ...
  def config_optimizer(self):
    ...
    if flags.type.lower() == 'sgd':
      self.optimizer = torch.optim.SGD(
          parameters, lr=base_lr, weight_decay=flags.weight_decay, momentum=flags.momentum)
  ...
  def train_epoch(self, epoch):
    ...
    for it in tqdm(rng, ncols=80, leave=False, disable=self.disable_tqdm):
      ...
      if self.is_master and log_per_iter > 0 and it % log_per_iter == 0:
        notes = 'iter: %d' % it
        train_tracker.log(epoch, msg_tag='- ', notes=notes, print_time=False, print_memory=True, log_file=os.path.join(self.FLAGS.SOLVER.logdir, "logs.txt"))
